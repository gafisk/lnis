...
if (isset($_POST['submit'])) {
  $user = $_POST['username'];
  $pass = $_POST['password'];
  $login = query("SELECT * from user where username='$user' and password='$pass' and level='superadmin'");
  ...
  if ($user == "" || $pass == "") {
    ...
    if ($check > 0) {
      ...
      if ($user == $fetch_data['username'] && $pass == $fetch_data['password'] && $fetch_data['level'] == 'superadmin') {
